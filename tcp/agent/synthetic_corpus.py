...
_CATEGORY_FLAGS: dict[str, int] = {
    ...
    "network-auth-write": CapabilityFlags.SUPPORTS_NETWORK
...
def build_synthetic_corpus(*, seed: int = 42) -> list[CorpusEntry]:
    ...
    for family in _FAMILIES:
        ...
        for name, commands, description, category in family["tools"]:
            ...
            descriptor = CapabilityDescriptor(
                ...
                output_formats=[
                    FormatDescriptor(
                        name=out_fmt, type=_FMT_MAP.get(out_fmt, FormatType.TEXT)
                    )
                ],
            ...
            entries.append(
                CorpusEntry(descriptor=descriptor, source=source, category=category)
            )
    ...
    for variant in ("v2", "v3", "beta"):
        for entry in base_entries:
            ...
            variant_cmds = [
                CommandDescriptor(name=f"{c.name}_{variant}") for c in d.commands
            ]
            ...
            entries.append(
                CorpusEntry(descriptor=vd, source=entry.source, category=entry.category)
            )
